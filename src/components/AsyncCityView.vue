<template>
    <div class="flex flex-col flex-1 items-center">
        ...
        <div v-if="route.query.preview" class="text-white p-4 bg-weather-secondary w-full text-center">
            <p>You Are in preview mode, click the + button to add this city to your list</p>
        </div>

        <!-- Weather Overview -->
        <div class="flex flex-col items-center text-white py-12">
            <h1 class="text-4xl font-bold text-white">{{ route.params.city }}</h1>
...
const getWeatherData = async () => {
    try {
        const weatherData = await axios.get(
            `https://api.openweathermap.org/data/2.5/onecall?units=metric&lat=${route.query.lat}&lon=${route.query.lon}&exclude={part}&appid=1fbbdd6c3a2247f9a40286269cf58241`
        );
